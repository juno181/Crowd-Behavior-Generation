...
class GroupNormLinear(nn.GroupNorm):
    def forward(self, x):
        x = x.transpose(1, -1)
        x = super().forward(x)
        x = x.transpose(1, -1)
...
class SquashLinear(Module):
    def __init__(self, dim_in, dim_out, dim_ctx):
        super(SquashLinear, self).__init__()
        self._layer = nn.Linear(dim_in, dim_out)
        self._hyper = nn.Linear(dim_ctx, dim_out)
...
class ConcatScaleLinear(Module):
    def __init__(self, dim_in, dim_out, dim_ctx):
...
class social_transformer(Module):
	def __init__(self, in_feat, out_feat, hid_feat=2048, n_head=4, n_layers=2):
		super(social_transformer, self).__init__()
		self.encoder = nn.Linear(in_feat, out_feat, bias=False)
		self.layer = nn.TransformerEncoderLayer(d_model=out_feat, nhead=n_head, dim_feedforward=hid_feat, batch_first=True)
		self.transformer_encoder = nn.TransformerEncoder(self.layer, num_layers=n_layers)

	def forward(self, h, mask=None, padding_mask=None):
		h_feat = self.encoder(h)
		h_feat_ = self.transformer_encoder(h_feat, mask, src_key_padding_mask=padding_mask)
		h_feat = h_feat + h_feat_

		return h_feat


class st_encoder(Module):
	def __init__(self):
		super().__init__()
		channel_in = 6
		channel_out = 32
